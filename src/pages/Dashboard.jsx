  ...
  const loadMonthBookings = async (bizId, startStr, endStr) => {
    try {
      ...
      const { data, error } = await supabase
        .from("bookings")
        .select(
          "id, date, hour, customer_name, service_name, status, deposit_receipt_path, deposit_status, deposit_paid, deposit_amount"
        )
  ...
  const calculateOccupation = async (businessId, dateStr, biz) => {
    try {
      const dayName = new Date(dateStr)
        .toLocaleDateString("es-UY", { weekday: "long              <div className="rounded-2xl border border-white/10 bg-slate-900/50 overflow-hidden">
  ...
  return (
    <div className="rounded-3xl bg-slate-900/70 border border-white/10 backdrop-blur-xl shadow-[0_18px_60px_rgba(0,0,0,0.6)] p-5">
      <div className="flex items-center justify-between mb-2">
        ...
        {pill ? (import { useEffect, useMemo, useState } from "react";
import { supabase } from "../supabaseClient";
import { useNavigate } from "react-router-dom";
import toast from "react-hot-toast";
...
export default function Dashboard() {
  ...
  // ✅ fin EXCLUSIVO (primer día del mes siguiente) para no perder turnos por horas/timezone
  ...
  const loadMonthBookings = async (bizId, startStr, endExclusiveStr) => {
    try {
      setCalendarLoading(true);
      ...
      const { data, error } = await supabase
        .from("bookings")
        .select(
          "id, date, hour, customer_name, service_name, status, deposit_receipt_path, deposit_status, deposit_paid"
        )
        .eq("business_id", bizId)
        .gte("date", startStr)
  ...
  useEffect(() => {
    if (!business?.id) return;
    loadMonthBookings(business.id, monthStartStr, monthEndExclusiveStr);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [business?.id, monthStartStr, monthEndExclusiveStr]);
  ...
  ───────────────────────────── */
  const openProof = (booking) => {
    const url = booking?.deposit_receipt_path;
    if (!url) {
      toast.error("Este turno no tiene comprobante.");
      return;
    }
