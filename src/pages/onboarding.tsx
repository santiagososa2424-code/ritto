      ...
      <div className="page">
        ...
        {step === 1 && (
          <div className="card">
            ...
            <div className="option-grid">
              <div className={`option-card${platform === 'excel' ? ' selected' : ''}`} onClick={() => { setPlatform('excel'); setSheetsNote(false); }}>
              ...
              <div className={`option-card${platform === 'sheets' ? ' selected' : ''}`} onClick={() => { setPlatform('sheets'); setSheetsNote(true); }}>
                <div className="option-icon">📋</div>
            ...
            {sheetsNote && (
              <div className="note-box">La conexión directa con Google Sheets ya viene. Mientras tanto, descargás el CSV desde Ritto y lo abrís en tu Sheets en segundos.</div>
            )}
            <button className="btn-primary" disabled={!platform} onClick={() => setStep(2)}>Siguiente →</button>
          </div>
        )}
        {step === 2 && (
          <div className="card">
            ...
            <div className="card-sub">Si ya tenés tu Excel o Sheets armado con nombres de columnas, Ritto puede usar exactamente esos nombres.</div>
            <div className="option-grid">
            ...
            <div className="btn-row">
              ...
              <button className="btn-primary" disabled={!hasColumns} onClick={() => setStep(hasColumns === 'yes' ? 3 : 4)}>Siguiente →</button>
            </div>
          </div>
        )}
        {step === 3 && (
          <div className="card card-wide">
            ...
            <div className="card-sub">Poné los mismos nombres que tiene tu planilla en la primera columna.</div>
            <div className="tip-box"><strong>Ejemplo:</strong> si tu planilla tiene una columna que se llama "Empresa proveedora", escribís eso exactamente igual.</div>
        ...
        {step === 4 && (
          <div className="card" style={{ textAlign: 'center' }}>
            ...
            <div className="card-sub" style={{ textAlign: 'center', marginBottom: 24 }}>Ritto está configurado. Empezá subiendo tu primera factura.</div>
            <div className="final-list" style={{ textAlign: 'left' }}>
              ...
              <div className="final-item"><div className="final-check"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#0a7c59" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg></div><span>Descargás el Excel con {hasColumns === 'yes' ? 'las columnas que configuraste' : 'la plantilla básica de Ritto'}</span></div>
            </div>
            <button className="btn-primary" disabled={saving} onClick={() => finish(hasColumns === 'yes' ? excelColumns : DEFAULT_COLUMNS)}>
