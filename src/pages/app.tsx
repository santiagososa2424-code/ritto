...
export default function AppPage() {
  ...
  async function saveInvoice(inv: ExtractedInvoice) {
    ...
    await supabase.from('invoices').insert({
      ...
      nro_documento: inv.nroDocumento,
      tipo_documento: inv.tipoDocumento,
      moneda: inv.moneda ?? 'UYU',
      neto: inv.neto ?? null,
      iva10: inv.iva10 ?? null,
      iva22: inv.iva22 ?? null,
      iva_total: inv.ivaTotal ?? null,
  ...
  async function processFiles(files: File[]) {
    const supported = files.filter((f) => {
      ...
      return name.endsWith('.xml') || name.endsWith('.pdf') || f.type.startsWith('image/');
    });
  ...
  async function exportExcel() {
    ...
    a.href = url;
    a.download =
      res.headers.get('Content-Disposition')?.split('filename=')[1] ?? 'facturas.xlsx';
    a.click();
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .user-email { font-size: 13px; color: var(--gray); }
        .btn-signout {
          background: none;
        ...
        .source-tag { display: inline-block; padding: 2px 8px; border-radius: 5px; font-size: 11px; font-weight: 600; }
        ...
        .status-processing { display: inline-flex; align-items: center; gap: 6px; color: var(--gray); font-size: 12px; }
        .spinner {
        ...
        .processing-bar {
          ...
          font-size: 14px; color: var(--green); margin-bottom: 16px; font-weight: 500;
        }
      `}</style>
        ...
        <div>
          <div className="section-header">
            ...
            {invoices.length > 0 && (
              <span className="count-badge">
                {done.length} procesada{done.length !== 1 ? 's' : ''}
              </span>
            )}
          </div>
          ...
          <div className="table-wrap">
            {loadingHistory ? (
              ...
              <div className="empty-state">
                Todavía no procesaste ninguna factura.<br />
                Soportamos imágenes, PDFs y XMLs (CFE del mail).
              </div>
            ) : (
              <table>
                <thead>
