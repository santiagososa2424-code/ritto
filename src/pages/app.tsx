    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nav { background: rgba(245,245,247,0.95); backdrop-filter: blur(14px); border-bottom: 1px solid var(--border); padding: 0 1.25rem; display: flex; align-items: center; justify-content: space-between; height: 54px; position: sticky; top: 0; z-index: 100; }
        .logo { font-family: 'DM Serif Display', serif; font-size: 20px; color: var(--green); }
        .nav-right { display: flex; align-items: center; gap: 8px; }
        .btn-icon { background: none; border: 1px solid var(--border); width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--gray); flex-shrink: 0; }
        .btn-icon:hover { background: var(--bg); }
        .btn-export { background: var(--green); color: #fff; border: none; padding: 8px 16px; border-radius: 8px; font-family: 'Figtree', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 5px; white-space: nowrap; }
        .btn-export:disabled { opacity: 0.4; cursor: not-allowed; }
        .main { max-width: 1000px; margin: 0 auto; padding: 24px 1.25rem 60px; }
        .upload-zone { border: 2px dashed var(--border); border-radius: 14px; padding: 44px 20px; text-align: center; background: var(--white); cursor: pointer; transition: border-color 0.15s, background 0.15s; margin-bottom: 24px; -webkit-tap-highlight-color: transparent; }
        ...
        .processing-bar { background: var(--green-light); border: 1px solid rgba(10,124,89,0.2); border-radius: 10px; padding: 11px 14px; display: flex; align-items: center; gap: 10px; font-size: 14px; color: var(--green); margin-bottom: 14px; font-weight: 500; }
      ...
      <main className="main">
        ...
        >
          ...
          <input ref={inputRef} type="file" multiple accept=".xml,.pdf,.jpg,.jpeg,.png,.webp" style={{ display: 'none' }}
        ...
        <div className="table-wrap">
          {loadingHistory ? (
            <div className="empty-state"><div className="spinner" style={{ margin: '0 auto 10px' }} />Cargando…</div>
          ) : invoices.length === 0 ? (
            <div className="empty-state">Todavía no subíste ninguna factura.<br />Soportamos imágenes, PDFs y XMLs de CFE.</div>
          ) : (
            <div className="table-scroll">
              <table>
                <thead>
                  <tr>
                    <th>Archivo</th><th>Tipo</th><th>Proveedor</th><th>RUT</th><th>Fecha</th>
                    <th>Neto</th><th>IVA</th><th>Total</th><th>Estado</th><th />
                  </tr>
                </thead>
                <tbody>
                  {invoices.map((inv) => (
                    <tr key={inv.id}>
                      ...
                      <td><span className={`source-tag source-${inv.source === 'cfe_xml' ? 'cfe' : inv.source}`}>{sourceLabel(inv.source)}</span></td>
                      ...
                      <td style={{ fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{inv.neto != null ? `${inv.moneda ?? 'UYU'} ${fmt(inv.neto)}` : '—'}</td>
                      <td style={{ fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>{inv.ivaTotal != null ? fmt(inv.ivaTotal) : '—'}</td>
                      <td style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 600, whiteSpace: 'nowrap' }}>{inv.total != null ? `${inv.moneda ?? 'UYU'} ${fmt(inv.total)}` : '—'}</td>
                      <td>
                      ...
                      <td><button className="btn-remove" onClick={() => deleteInvoice(inv.id)}>×</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
